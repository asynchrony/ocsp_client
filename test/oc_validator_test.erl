...
validate_cert_returns_error_when_revoke_message_returned_from_provider() ->
    ...
    ?assertMatch({error, revoked}, oc_validator:validate_cert(peercert, "provider url")),
...
validate_cert_returns_ok_when_good_returned_from_provider() ->
    ...
    ?assertMatch(ok, oc_validator:validate_cert(peercert, "provider url")),
...
validate_cert_returns_error_when_provider_request_returns_error() ->
    ...
    ?assertMatch({error, "reason"}, oc_validator:validate_cert(peercert, "provider url")),
...
validate_cert_returns_error_when_provider_request_returns_non_200_http_code() ->
    ...
    ?assertMatch({error, {500, description, error_response}}, oc_validator:validate_cert(peercert, "provider url")),
