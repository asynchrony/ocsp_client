...
validate_should_return_ok_when_cert_status_is_good_test() ->
    ...
    ?assertMatch(ok, oc_response_validator:validate(Response, ?GOOD_RESPONSE_NONCE)).
...
validate_should_return_error_when_response_status_not_successful_test() ->
    ...
    ?assertMatch({error, {ocsp, {responseStatus, internalError}}}, oc_response_validator:validate(Response, nonce)).
...
validate_should_return_error_when_unhandled_response_type_test() ->
    ...
    ?assertMatch({error, {ocsp, unhandled_response_type}}, oc_response_validator:validate(Response, nonce)).

validate_should_return_error_when_cert_status_is_revoked_test() ->
    Response = test_support:read_data("revoked_response.der"),
    ?assertMatch({error, certificate_revoked}, oc_response_validator:validate(Response, ?REVOKED_RESPONSE_NONCE)).

validate_should_return_error_when_cert_status_is_unknown_test() ->
    Response = test_support:read_data("unknown_response.der"),
    ?assertMatch({error, certificate_unknown_by_ocsp}, oc_response_validator:validate(Response, ?UNKNOWN_RESPONSE_NONCE)).

validate_should_return_error_when_nonce_does_not_match_test() ->
    Response = test_support:read_data("good_response.der"),
    ?assertMatch({error, {ocsp, nonce_mismatch}}, oc_response_validator:validate(Response, ?UNKNOWN_RESPONSE_NONCE)).
